...
const char* doc_root = "/home/corn/web_server/resource";
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text) // 解析请求首行
{
    ...
    if (result[1] != "GET")
        return BAD_REQUEST;
    int j = 0;
    for(string::const_iterator i = result[2].first; i != result[2].second;i++)
    {
        m_url[j] = *i;
        j ++;
    }m_url[j]= '\0';
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text) //解析请求头
{
    ...
    regex pattern_Connction("Connection:[ \t](.*)");
    regex pattern_Blank_line("(\n\\s*\n)$");


    if(regex_search(iterstart, iterend,result,pattern_Connction)){
        if(result[1] == "keep-alive")
            m_linger = true;
    }
...
bool http_conn::write() {
    ...
    while(1){
        ...
        if ( bytes_to_send  <= 0) {
            ...
            if( m_linger){ // 如果保持连接，则init（）设置状态为解析响应行
                init();
                return true;
            }else{
                return false;
...
bool http_conn::add_content_length(int content_len) {
    return add_response( "Connection-Length: %d\r\n", content_len);
}
