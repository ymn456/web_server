...
class http_conn{
    public:
        ...
        // HTTP请求方法，这里只支持GET
        enum METHOD {GET = 0, POST, HEAD, PUT, DELETE, TRECE, OPTIONS, CONNECT};
    ...
    private:
        ...
        char m_read_file[FILENAME_LEN];     // 客户请求的目标文件的完整路径，其内容等于 doc_root + m_url， doc_root 是网站根目录
        char* m_host;                       // 主机名
        bool m_linger;                      // HTTP是否要保持连接
        char m_url[FILENAME_LEN];                       // 请求目标文件名
        char* m_version;                    // 协议版本
        int m_content_length;               // HTTP请求的消息总长度

        char m_write_buf[ WRITE_BUFFER_SIZE]; // 写缓冲区
        struct stat m_file_stat;            // 目标文件的状态，以此判断文件是否存在， 是否为目录，是否可读， 并获取文件大小等信息
        char* m_file_address;               // 目标文件被映射到内存中的地址
        int m_write_index;                  // 写缓冲区中待发送的字节数
        struct iovec m_iv[2];   // writve执行写操作
